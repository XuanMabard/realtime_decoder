...
int TrialforDecision = 0 %(DS) from this trial, start decision trial. for guided, make it 100; for decision, make it 0
int reward_window_outer_arm = 8000 % (DS) window of outer arm reward 
...
int deliverPeriodOuter= 400   		% how long to deliver the reward at outer wells, 150 uL - 400; 100uL - 275
...
int beep_delay = 0 % additional delay in beep sound
...
portout[7] = 1 %center
portout[5] = 0 % back; changed from 10 to 5
%portout[12] = 0
%portout[13] = 0
;
...
function 14
	...
	%	%taskstate = 3
	%	disp('TASKSTATE3')
	if (trialtype == 1 && content_generation_avail == 1 && outer_reward_window_done == 1) do
...
%		disp('taskstate is 3 end epoch')
	...
	else if (taskstate == 2 && trialtype == 1 && reward_available_at_center > 0) do
...
callback portin[9] up
	...
	if currWell != lastWell do
		...
		if (taskstate == 2 && target_location == 1 && reward_avail == 1 && reward_delivered == 0) do
			...
			disp('WHITENOISE')
			reward_avail = 0	
			reward_available_at_center = 2 % meaning that center does not give reward
		else if (taskstate == 2 && reward_avail == 0 ) do
			disp('WHITENOISE')
			contentOuterCount = contentOuterCount + 1
			wrong_outer_poke = 1
