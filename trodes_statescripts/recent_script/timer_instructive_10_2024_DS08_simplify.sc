...
function 18
	...
	do in content_trial_time
		disp('CURRENT BEEP')
		if (wrong_outer_visit == 0 && reward_available_at_center == 0 && decision_timeout ==0 && reward_available_out_if_poke == 0) do
			disp('BEEP READY')
...
% first content trial
function 13
	trigger(19) % start the TS2 duration timer 
	portout[7] = 1
	reward_delivered = 0
...
callback portin[7] up % center well
	...
	% taskstate == 2
		...
		% other 
			...
			if (reward_available_out_if_poke == 1) do
				disp('NEXT_TRIAL')
