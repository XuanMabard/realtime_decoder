...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
int TrialforDecision = 100 %(DS) from this trial, start decision trial. for guided, make it 100; for decision, make it -1
...
int arm3_repeat_time = 5000 % if the rat is not proximate during timer -- check again in 5 sec
...
int num_arm3_trials = 0
int is_center_well_proximate = 0
...
% function to deliver reward to wells box
...
function 8
	do in beep_delay
		...
		else if (target_location == 3) do
			portout[4]=1
			disp('start content trial of TARGET ARM3')
			num_arm3_trials = num_arm3_trials + 1
		end	
		disp('BEEP3')
...
function 18
	if (content_generation_avail == 0) do
		disp('NEW TRIAL')	
	end
	...
	do in content_trial_time
		trigger(42) % same as disp('CURRENT BEEP')
		if (wrong_outer_visit == 0 && reward_available_at_center == 0 && decision_timeout ==0 && reward_available_out_if_poke == 0 && content_generation_avail == 1) do
			reward_available_out_if_poke = 0
			if (target_location == 3) do
				if (is_center_well_proximate ==1) do
					trigger(37)
					trigger(30)
				else do
				 	while (is_center_well_proximate == 0) do every arm3_repeat_time
						trigger(42) % same as disp('CURRENT BEEP')
			 		then do
						trigger(37)
						trigger(30)
...
function 13
	...
	do in content_trials_time_limit
		...
		trigger(41) % Same as disp('TERMINATE EPOCH')
	end
...
function 38
	is_center_well_proximate = 1
end

function 39
	is_center_well_proximate = 0
end
...
callback portin[15] up
	...
	if currWell != lastWell do
		...
		if (taskstate == 2 && target_location == 2 && reward_avail == 1 && reward_delivered == 0) do
			...
			trigger(40) %  equals to disp('WRONG OUTER VISIT')
		end
