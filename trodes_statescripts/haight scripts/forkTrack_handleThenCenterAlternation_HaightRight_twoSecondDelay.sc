% PROGRAM NAME: 	Fork track handle then center alternation with two second delay. Right side of Haight. Nov 2020.
% AUTHOR: 		JAG
...
int centerWellPump=centerWell+8
int lastSideWell=0
int lastWell=0
int currWell=0
int rewardWell=0
...
int ch_zeroOne=1 % 0 for center alt, 1 for handle alt
...
% CALLBACKS
callback portin[21] up % poke at center
	disp('center_poke')
	currWell=centerWell
...
callback portin[21] down % set lastWell to center
	lastWell=centerWell
end

callback portin[24] up % poke at handle
	disp('handle_poke')
	currWell=handleWell
