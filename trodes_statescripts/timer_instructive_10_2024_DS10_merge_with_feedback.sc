...
int content_trials_time_limit = 2100000 %35mins		% box time 2400000 = 40 min; 1800000 = 30min, 1200000 = 20min
int content_trials_limit = 60 % if center port does not give reward all the time.
...
int lockoutPeriod_centerReward = 3000  %(DS) if the BEEP sound happens right after the rat started to consume reward, it won't react
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5
...
function 14
	...
	if (trialtype == 1 && content_generation_avail == 1 && outer_reward_window_done == 1 && accept_scm == 1) do
		if (target_location == 1 || target_location == 3) do %based on the target location
			target_location = 1
			trigger(8)		
		end
	end
...
function 6 % ARM 2 shortcut message
	...
	if (trialtype == 1 && content_generation_avail == 1 && outer_reward_window_done == 1 && accept_scm == 1) do
		if (target_location == 2 || target_location == 3) do %based on the target location
			target_location = 2
			trigger(8)		
		end
	end
