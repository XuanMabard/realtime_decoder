...
int content_trials_time_limit = 2400000		% box time 1800000 = 30min, 40 mins = 2400000, 35 = 2100000
int reward_avail = 0
...
% new: only run if trialtype = 3
function 14
	if content_trials_finish == 1 do
		taskstate = 3
		disp('TASKSTATE3')
	else if (trialtype == 3 && content_pause == 0) do
	%else do
		trialtype = 2
		contentTrialCount = contentTrialCount + 1
		disp('start content trial')
...
callback portin[7] up % center well
	...
	% for content trials, deliver 1 reward when he returns
	else if (taskstate == 2 && trialtype == 1) do
		if lastWell != currWell do
			disp('start instructive trial')
			content_pause = 1
			proximity = 1
			do in waittime
				if proximity > 0 do
			...
			% wait 5 sec before sound cue for content
			do in 100
				content_pause = 0
			end
		else do 
