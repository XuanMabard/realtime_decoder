...
int trial_start = 0					% try to prevent trigger(18) from mutliple starts
int post_reward_time = 0			% try to prevent mutliple rewards in single trial
...
int content_trials_time_limit = 900000		% box time 1800000 = 30min, 40 mins = 2400000, 35 = 2100000
int reward_avail = 0
...
function 18
	...
	%	trigger(19)
	%end
	% this prevents starting next trial for 3 sec
	%trial_start = 1
	%do in 500
	%	trial_start = 0
	%end
...
function 7
	portout[2] = 0
	content_lockout = 1
	disp(content_lockout)
	do in 15000
		portout[2] = 1
		content_lockout = 0
		disp(content_lockout)
		disp('content lockout done')
		trigger(18)
	end
end;
...
callback portin[2] up % center well
	...
	% for nosepoke version
	else if (taskstate == 2 && trialtype == 1 && content_lockout == 0) do
		%contentReward = contentReward + 1
		...
		if proximity == 0 do
			disp('content wait time')
			proximity = 1
			do in waittime
				if proximity > 0 do
					proximity = 0
					%trialtype = 2
					%if content_lockout == 0 do
					disp('BEEP3')
					disp('BEEP4')
					contentReward = contentReward + 1
					disp('next trial: following reward')
					trigger(18)
					% prevent multiple rewards in one trial
					%post_reward_time = 1
					%do in 5000
					%	post_reward_time = 0
					%end						
...
callback portin[2] down
	...
	% i think this will start a new trial if he has left center for too long - or above in function 16???
	% if he stays nosepoked, see port[2] up above
	%remove lockouts
