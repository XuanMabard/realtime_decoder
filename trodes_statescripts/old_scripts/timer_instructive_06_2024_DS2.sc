...
int content_trials_time_limit = 1800000		% box time 1800000 = 30min, 1200000 = 20min
int reward_avail = 0
...
int contentCenterReward = 0
int contentDecision = 0
...
function 5
	...
	disp(contentCenterReward)
	disp(contentDecision)
...
function 16
	if content_trials_finish == 1 do
		...
		do in target_replay_wait % what is target_replay_wait?
			...
			%trigger(17) % make the window of reward -- no need for instructive task
...
callback portin[7] up % center well
	...
	else if (taskstate == 2 && trialtype == 1 && contentReward == 0 && reward_delivered == 0) do
		contentCenterReward = contentCenterReward + 1
...
callback portin[15] up
	...
	if currWell != lastWell do
		...
		if (taskstate == 2 && target_location == 2 && reward_avail == 1 && reward_delivered == 0) do
			contentDecision = contentDecision + 1
			disp('REWARD ARM2')
			reward_delivered = 1
...
callback portin[15] down
	...
	if (taskstate == 2 && target_location == 2 && reward_avail == 1 && reward_delivered == 1) do
		portout[15] = 0
		portout[7] = 1
				

	else if (taskstate == 2 && target_location == 1 && reward_avail == 0 && reward_delivered == 0) do
		portout[9] = 0
		portout[7] = 1
	end
...
callback portin[9] down
	...
	if (taskstate == 2 && target_location == 1 && reward_avail == 1 && reward_delivered == 1) do
		portout[9] = 0
		portout[7] = 1
				
	else if (taskstate == 2 && target_location == 2 && reward_avail == 0 && reward_delivered == 0) do
		portout[15] = 0
		portout[7] = 1
	end
