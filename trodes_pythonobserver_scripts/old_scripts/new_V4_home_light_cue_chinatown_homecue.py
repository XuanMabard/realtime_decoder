...
def pokeIn(dio):
	...
	if taskState == 1:
		for num in range(len(cuedWells)):
			if currWell == cuedWells[num]:
				doOuter(num)
	else:
...
def endHome():
	...
	if taskState == 2 and outer_count_content == 0:
		...
		for num in range(len(cuedWells)):			# turn off outer lights
			print("SCQTMESSAGE: dio = "+str(cuedWells[num])+";\n")
...
def chooseGoal():
	...
	if taskState == 1:
		...
		valid_goals = [home_Goal<outerarm_required_rewards,
					   arm1_Goal<outerarm_required_rewards,
		...
		if cued_trial_counter > 0 and home_Goal < outerarm_required_rewards-1 and arm1_Goal < outerarm_required_rewards-1 and arm2_Goal < outerarm_required_rewards-1:
			print('goal',goalWell)
			print('old goal',oldGoal)
			print(cuedWells.index(oldGoal))
			valid_goals[cuedWells.index(oldGoal)] = False
		...
		print(list(compress(cuedWells,valid_goals)))
		...
		goalWell = np.random.choice(list(compress(cuedWells,valid_goals)),1,replace=False)
		oldGoal = goalWell
...
def doOuter(val):
	...
	if trialtype == 2:
		...
		if currWell in goalWell :  # repeated; reward
			if taskState == 1:
				print("SCQTMESSAGE: rewardWell = "+str(cuedPumps[val])+";\n")
			if taskState == 3:
			...
			if currWell == cuedWells[1]:
				arm1_Goal+=1
				#print('arm1',arm1_Goal)
			elif currWell == cuedWells[2]:
				arm2_Goal+=1
			...
			elif currWell == cuedWells[0]:
				home_Goal+=1
				#print('arm4',arm4_Goal)	
			print('arm1',arm1_Goal,'arm2',arm2_Goal,'arm3',arm3_Goal,'arm4',arm4_Goal,'home',home_Goal)
			...
			if (taskState == 1 and arm1_Goal>=outerarm_required_rewards and arm2_Goal>=outerarm_required_rewards
				and home_Goal>=outerarm_required_rewards):
				taskState = 2
