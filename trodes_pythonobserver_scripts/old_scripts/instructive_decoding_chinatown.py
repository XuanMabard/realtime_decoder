...
def beep_home():
	...
	# only want this for first content trial - after that want to deliver reward at outer well
	if taskState == 2:
...
# deliver reward at outer arm for content trials
def beep_outer():
	global centerPump
	global centerWell
	global trialtype
	...
	# in taskstate 2: deliver reward at outer well
	if taskState == 2:
		#deliver reward
		for num in range(len(outerWells)):
			if currWell == outerWells[num]:
				print("SCQTMESSAGE: rewardWell = "+str(outerPumps[num])+";\n")
				print("SCQTMESSAGE: trigger(1);\n")
				#print("SCQTMESSAGE: centerCount = centerCount + 1;\n") # update centercount in SC
...
def callback(line):
	...
	if line.find("BEEP3") >= 0:
		generate_beep()
	# for 1st content trials. reward only no beep sound
	if line.find("BEEP4") >= 0:
		beep_home()
	# for all other content trials
	if line.find("BEEP5") >= 0:
		beep_outer()
