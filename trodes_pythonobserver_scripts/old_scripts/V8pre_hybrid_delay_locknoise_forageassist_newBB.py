...
def chooseDelay():
    ...
    if RWcount[trialtype-1]<3:  #first 3 trials of of each type should be short
        return startwaitdist[RWcount[trialtype-1]]

    else:
        if RWcount[trialtype-1]<=10:  #trials 4-10 of each type will be avg of startwaitdist and normal waitdist
    ...
    if trialtype == 3:
        ...
        if currWell in goalWell :  # repeated; reward
            ...
            if thisGoal >= outerReps:  #maxed repeats reached, time to switch
                ...
                if numgoals < 2:    # if 1 goal only, use chooseGoal function to enforce no repeats
                    chooseGoal()
                else:
                    #goalWell = np.random.choice(validOuterWells,numgoals,replace=False)   #choose a different selection                    
                    goalWell = np.random.choice(outerWells,numgoals,replace=False)   #choose a different selection
                    print(goalWell)
...
def chooseGoal():    # this is only called when numgoals ==1. to assist forage, choose 2 goals; first encountered will turn into only goal
    global goalWell
    global oldGoals
    global numgoals
    global outerWells
    ...
    outerReps = np.random.randint(4,13) 
...
def lockout(val):   # turn off all lights for certain amount of time
    ...
    if locktype == 1:
        print("SCQTMESSAGE: locktype1 = locktype1 + 1;\n") # type 1 = wrong rip/wait well
    if locktype == 2:
        print("SCQTMESSAGE: locktype2 = locktype2 + 1;\n") # update lockcount in SC 
    if locktype == 3:
        print("SCQTMESSAGE: locktype3 = locktype3 + 1;\n")  # update rip/wait hold errors
...
def callback(line):
	...
	if line.find("BEEP2") >= 0: # make a beep and deliver reward
		generate_beep()
	if line.find("CLICK1") >= 0: # make a click and deliver reward
		click()
	if line.find("CLICK2") >= 0: # make a click and deliver reward
		generate_click()
...
homeWell = 10
waitWells = [11,12]
outerWells = [1,2,3,4,5,6,7,8]
#validOuterWells = [8,11,12,14,15]  # for excluding their bias goals when reducing arm num
# define pumps
homePump = 25
waitPumps = [26, 27]
...
trialtype = 0
outerReps = 15
#outerReps = np.random.randint(4,13) #10
print("SCQTMESSAGE: disp('outerreps = "+str(outerReps)+"');\n") 

allGoal = 0
thisGoal = 0
numgoals = 1 # number of outer arms rewarded
forageNum = 2
if numgoals == 1:
    goalWell = np.random.choice(outerWells,forageNum,replace=False)   #initialize to 4 wells
else:
    goalWell = np.random.choice(outerWells,numgoals,replace=False)
    #goalWell = np.random.choice(validOuterWells,numgoals,replace=False)

oldGoals = []   #initialize with any wells to exclude from being goal
waitdist = [3000, 3500, 3000, 4000, 4500, 5500, 7500, 6000, 7000, 7500, 6500]
#waitdist = [1000, 1000, 1500, 1200, 1300, 1400, 1500, 1500]

startwaitdist = [1000, 2000, 1500]
